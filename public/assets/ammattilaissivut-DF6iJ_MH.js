...
`,i.appendChild(t)}),_()},c=async()=>{if(o)try{let t=await e(`https://ptsdjahyvinvointiseurantasovellus.polandcentral.cloudapp.azure.com/api/users/patients/${o}`,{method:`GET`,headers:{"Content-Type":`application/json`,Authorization:`Bearer ${localStorage.getItem(`token`)}`}}),n=t?.users||t?.data||t;if(!Array.isArray(n)){console.error(`API ei palauttanut taulukkoa:`,t);return}s(n),console.log(n)}catch(e){console.error(`Virhe haettaessa potilaita:`,e)}},l=async t=>{let n=localStorage.getItem(`token`);return await e(`https://ptsdjahyvinvointiseurantasovellus.polandcentral.cloudapp.azure.com/api/users/${t}`,{headers:{Authorization:`Bearer ${n}`}})},u=async t=>{let n=localStorage.getItem(`token`);return await e(`https://ptsdjahyvinvointiseurantasovellus.polandcentral.cloudapp.azure.com/api/tsq/user/${t}`,{headers:{Authorization:`Bearer ${n}`}})},d=async t=>{let n=localStorage.getItem(`token`);return await e(`https://ptsdjahyvinvointiseurantasovellus.polandcentral.cloudapp.azure.com/api/tsq/${t}`,{headers:{Authorization:`Bearer ${n}`}})},f=(e,t,n,r)=>{let i=document.querySelector(`.diary_dialog`),a=i.querySelector(`.patient-info`),o=i.querySelector(`.tsq-info`);console.log(`TSQ DATA:`,t),a.innerHTML=`
...
`:e.innerHTML=`<p>Ei vastauksia</p>`,n.textContent=`Piilota TSQ-vastaukset`}catch(t){console.error(t),e.innerHTML=`<p>TSQ-vastausten haku epäonnistui</p>`}return}if(!t)return;let r=t.closest(`.entry-card`).querySelector(`.entry-details`),i=r.classList.contains(`hidden`);r.classList.toggle(`hidden`),t.textContent=i?`Piilota vastaukset`:`Näytä vastaukset`});let l=(Array.isArray(r)?r:r?.results||[])[0];if(l){let e=Math.round(l.readiness??0),t=Math.round(l.stress_index??0);o.innerHTML+=`
  ...
  `}i.showModal()},p=async()=>await e(`https://ptsdjahyvinvointiseurantasovellus.polandcentral.cloudapp.azure.com/api/users/patients`,{headers:{Authorization:`Bearer ${localStorage.getItem(`token`)}`}}),m=async t=>{let n=localStorage.getItem(`token`);return await e(`https://ptsdjahyvinvointiseurantasovellus.polandcentral.cloudapp.azure.com//api/users/patients/${t}/doctor`,{method:`PUT`,headers:{"Content-Type":`application/json`,Authorization:`Bearer ${n}`}})},h=async t=>{let n=localStorage.getItem(`token`);return await e(`https://ptsdjahyvinvointiseurantasovellus.polandcentral.cloudapp.azure.com/api/entries/user/${t}`,{headers:{Authorization:`Bearer ${n}`}})},g=async t=>{let n=localStorage.getItem(`token`);return await e(`https://ptsdjahyvinvointiseurantasovellus.polandcentral.cloudapp.azure.com/api/kubios/user-data/${t}`,{headers:{Authorization:`Bearer ${n}`}})},_=()=>{document.querySelectorAll(`.info-btn`).forEach(e=>{e.addEventListener(`click`,async e=>{let t=e.target.dataset.id,n=document.querySelector(`.diary_dialog`),r=n.querySelector(`.patient-info`),i=n.querySelector(`.tsq-info`);r.innerHTML=`<p>Ladataan...</p>`,i.innerHTML=``,n.showModal();try{let e=await l(t),n=await u(t),i=await h(t),a=await g(t);if(!e||e.error){r.innerHTML=`<p>Virhe potilaan haussa</p>`;return}f(e,n,i,a)}catch(e){console.error(e),r.innerHTML=`<p>Virhe haussa</p>`}})})},v=e=>{let t=document.querySelector(`.all-patients-dialog`),n=t.querySelector(`.all-patients-list`);if(!e||e.length===0){n.innerHTML=`<p>Ei potilaita</p>`,t.showModal();return}n.innerHTML=e.map(e=>`
  ...
  `).join(``),t.showModal(),n.querySelectorAll(`.add-patient-btn`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.id;try{await m(t),e.disabled=!0,e.textContent=`Lisätty`}catch(e){console.error(e),alert(`Potilaan lisääminen epäonnistui`)}})})},y=()=>{let e=document.querySelector(`.all-patients-dialog`),t=document.querySelector(`#close-all-patients`);!e||!t||t.addEventListener(`click`,()=>{e.close()})},b=()=>{let e=document.querySelector(`.diary_dialog`),t=document.getElementById(`close-dialog`);!e||!t||t.addEventListener(`click`,()=>{e.close()})},x=document.querySelector(`.get_all_patients`);a&&a.addEventListener(`click`,c),x&&x.addEventListener(`click`,async()=>{try{let e=await p();v(e.users||e.data||e)}catch(e){console.error(e)}}),document.addEventListener(`DOMContentLoaded`,()=>{b(),y()});